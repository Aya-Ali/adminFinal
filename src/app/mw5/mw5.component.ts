...
@Component({
  selector: 'app-mw5',
  templateUrl: './mw5.component.html',
  styleUrls: ['./mw5.component.css']
})
export class Mw5Component implements OnInit {
  public product: any = [];
  public sells: any = [];
  ...
  selectOption(e) {
    ...
    this.sells = JSON.parse(JSON.stringify(this.sellsCopy)).filter((obj) => {
      return obj.sell_lines = JSON.parse(JSON.stringify(obj.sell_lines)).filter((element) => {
        let sellNote = (new Function("return [" + (element.sell_line_note) + "];")())
        if (sellNote.length > 0) {
          if (Category == '*') {
            return sellNote[0].type != 0
          }
          else if (Category == 'G') {
            return (sellNote[0].type != 0 && sellNote[0].type != 1)
          }
          else if (Category == 'M') {
            return sellNote[0].type == 1
          }
        }
        else {
          return false
  ...
  filter(sells) {
    ...
    sells.forEach(sell => {
      sell.sell_lines.forEach((sellLine) => {
        ...
        if (sellNote.length > 0) {
          let index = this.Filtersells.findIndex((obj) => obj.contact_id == sell.contact_id)
          if (index > -1) {
            let index2 = this.Filtersells[index].products.findIndex((obj) => obj.name == sellNote[0].name)
            if (index2 > -1) {
              ...
              this.Filtersells[index].products.push({
                name: sellNote[0].name, avgUnit_price: (sellLine.quantity * sellLine.unit_price)
              });
            ...
            this.Filtersells.push({
              contact_id: sell.contact_id, total: sellLine.quantity * sellLine.unit_price, products: [{
                name: sellNote[0].name, avgUnit_price: sellLine.quantity * sellLine.unit_price
              }]
            })
          }
  ...
  printData() {
    ...
    newWin.document.write(`<div style="text-align:center"><h1 >${this._langChange.mw5}</h1><span>${this.startDate}</span> - <span>${this.endDate}</span></div>` + divToPrint.outerHTML + `<p style="text-align:center;">© Copyright 2020
