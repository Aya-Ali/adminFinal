...
@Component({
  selector: 'app-mw2',
  templateUrl: './mw2.component.html',
  styleUrls: ['./mw2.component.css']
})
export class Mw2Component implements OnInit {
  public sells: any = [];
        ...
        sells.forEach(sell => {
          if (sell.contact_id == element.id) {
            sell.sell_lines.forEach((sellLine) => {
              ...
              if (sellNote.length > 0 && sellNote[0].type == 1) {
                let index = this.Filtersells.findIndex((obj) => obj.name == element.name)
                if (index > -1) {
                  this.Filtersells[index].cost += Number(sellLine.unit_price_inc_tax)
                }
                else {
                  this.Filtersells.push({ name: element.name, cost: Number(sellLine.unit_price_inc_tax) })
                }
              }
 ...
 logOut() {
  ...
  printData() {
    ...
    newWin.document.write(`<div style="text-align:center"><h3>${this._langChange.mw2}</h3><span>${this.startDate}</span> - <span>${this.endDate}</span></div>` + divToPrint.outerHTML + `<p style="text-align:center;">© Copyright 2020
  ...
  exportThisWithParameter(tableID: any, excelName: any) {
    var uri = 'data:application/vnd.ms-excel;base64,',
      template = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel"  xmlns="http://www.w3.org/TR/REC-html40"><head> <!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets> <x:ExcelWorksheet><x:Name>{worksheet}</x:Name> <x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions> </x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook> </xml><![endif]--></head><body><div style="text-align:center"><h3 >${this._langChange.mw2}</h3><span>${this.startDate}</span> - <span>${this.endDate}</span></div> <table>{table}</table>
